...
export class EventHub extends DurableObject {
  ...
  async initializeSql() {
    ...
    console.log('[EventHub] SQLite initialized for cloud events');
    return this.sql;
  }
  ...
  async saveCloudEvent(event: {
    ...
    try {
      ...
      console.error('[EventHub] Failed to save event:', err.message);
      return null;
    }
  }
  ...
  async getRecentEvents(scope: string, limit = 50): Promise<any[]> {
    try {
      ...
      console.error('[EventHub] getRecentEvents error:', err);
  ...
  async fetch(request: Request): Promise<Response> {
    ...
    if (upgradeHeader === 'websocket') {
      console.log('[EventHub] WebSocket upgrade request');
    ...
    if (request.method === "POST") {
      ...
      try {
        ...
        console.log('[EventHub] JSON parse error:', e.message);
      }
    ...
    try {
      ...
      if (eventData.opcode && eventData.streamid) {
        ...
        this.eventExists(eventData.opcode, eventData.streamid, scope, 5)
          ...
          .catch(err => console.error('[EventHub] Deduplication check failed:', err));
        return;
      }
    } catch (e) {}
